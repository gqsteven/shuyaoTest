...
var menuItem = Vue.extend({
	...
	template:[
	          '<li :class="{active: (item.type===0 && index === 0)}">',
				  '<a class = "hoverable" v-if="item.type === 0" href="javascript:;">',
					  '<i v-if="item.icon != null" :class="item.icon"></i>',
					  '<b><span>{{item.name}}</span></b>',
					  '<i class="fa fa-angle-left pull-right hoverable"></i>',
				  '</a>',
				  '<ul v-if="item.type === 0" class="treeview-menu">',
					  '<menu-item :item="item" :index="index" v-for="(item, index) in item.list"></menu-item>',
				  '</ul>',
				  '<a class = "hoverable" v-if="item.type === 1" :href="\'#\'+item.url">' +
					  '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i v-if="item.icon != null" :class="item.icon"></i>' +
	...
	for(var key in menuList){
		...
		if(menu.type == 0){
			...
			router.add('#'+menu.url, function() {
			    ...
			    vm.navTitle = $("a[href='"+url+"']").text().trim();
			});
		}
	}
}
